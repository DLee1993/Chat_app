...
io.on("connection", (socket) => {
    ...
    // upon connection send a message to the user that connected
    ...
    socket.on("enterRoom", ({ name, room }) => {
        ...
        // unable to update prev room user list until after the state update in the activate user
        if (prevRoom) {
        ...
        // broadcast to all others in the new room
        socket.broadcast
            .to(user.room)
            .emit("message", buildMessage(ADMIN, `${user.name} has joined`));
    ...
    // when user disconnects it will broadcast to all other users
